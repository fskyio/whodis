...
func (a *App) performLookup(ctx context.Context, query string, mode protocol) (lookupResult, error) {
	...
	default:
		...
		if rdapErr == nil || !shouldFallbackRDAP(rdapErr) {
...
func (a *App) lookupRDAP(ctx context.Context, query string) (lookupResult, error) {
	...
	if len(response.Body) > 0 || response.StatusCode != 0 || response.Truncated {
		item := resultItem{
	...
	output.TTL, output.Cacheable = rdapFreshness(response.Header, a.config.CacheTTL, a.now())
	return output, nil
}
